...
function VaccinePoint(props) {
  ...
  const handleDelete = async (record) => {
    if (!record.new) {
      try {
        const response = await axios.post('/api/DeleteVaccinationPlaces', {
          token: userToken,
        ...
        if (res.error !== 0) {
          notification.error({message: '提示', description: `删除疫苗点失败，错误码${res.error}，错误信息：${res.message}`})
        } else {
          notification.success({message: '提示', description: `删除疫苗点成功`})
        ...
        console.error(error);
        notification.error({message: '提示', description: `删除疫苗点失败，网络错误`})
      }
    } else {
      const newData = data.filter(item => item.place_id !== record.place_id);
      setData(newData);
  ...
  const handleAddUser = async (place_id) => {
    ...
    try {
      const response = await axios.post('/api/AddVaccinationPlaces', {
        token: userToken,
      ...
      const res = response.data
      if (res.error !== 0) {
        notification.error({message: '提示', description: `添加疫苗点失败，错误码${res.error}，错误信息：${res.message}`})
      } else {
        notification.success({message: '提示', description: `添加疫苗点成功，[debug]服务器返回id = ${res.place_id}`})
      ...
      console.error(error);
      notification.error({message: '提示', description: `添加疫苗点失败，网络错误`})
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      ...getColumnSearchProps('place_name', '疫苗点名称')
    },
    {
      title: '疫苗点纬度',
  ...
  const handleSave = async (row, column_name) => {
    ...
    if (changed) {
      ...
      if (!row.new) {
        try {
          const response = await axios.post('/api/SetVaccinationPlaces', {
            token: userToken,
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.post('/api/GetVaccinationPlacesAll', {
        token: userToken,
      })
      console.log(response);
      const res = response.data
  ...
  return (
    <>
      <Button type="primary" onClick={handleAdd}>添加疫苗点</Button>
